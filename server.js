...
createServer({
    ...
    routes() {
        ...
        this.get("/host/albums", (schema, request) => {
            // Hard-code the hostId for now
            return schema.albums.where({ hostId: "123" })
        })

        this.get("/host/albums/:id", (schema, request) => {
            // Hard-code the hostId for now
            const id = request.params.id
            return schema.albums.findBy({ id, hostId: "123" })
        })
    }
})